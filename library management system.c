#include<windows.h>				 	//by using it we can use verious macros and also get time in milisec etc
...
#include<dos.h>                     //contains _dos_getdate Delay,Getdate,Gettime,Nosound,Setdate,Sleep,Sound
...
FILE *fp,*ft,*fs;                //list of global file pointers that can be acceed form anywhere in program

COORD coord = {0, 0};            //using it for co-ordination
...
int s;
char findbook;
char password[10]={"TWINS"};     //password
...
void mainmenu()                                 //main manu after password
{
	system("cls");
	int i;
	gotoxy(20,3);
...
void deletebooks()            //function that delete items from file fp
{
	...
	int d;
	char another='y';
	while(another=='y')      //infinite loop because we can add unlimited item
	{
		system("cls");
		gotoxy(10,5);
		printf("Enter the Book ID to  delete:");
		scanf("%d",&d);
		fp=fopen("pankaj.dat","rb+");
		rewind(fp);
		...
		if(findbook=='t' )
		{
			...
			if(getch()=='y')
			{
				...
				fclose(fp);
				remove("pankaj.dat");
				rename("temp.dat","pankaj.dat");              //copy all item from temporary file to fp except that
				fp=fopen("pankaj.dat","rb+");                //we want to delete
				if(findbook=='t')
				{
					gotoxy(10,10);
...
void searchbooks()
{
	...
	gotoxy( 15,20);
	printf("Enter Your Choice");
	fp=fopen("pankaj.dat","rb+");           //open file for reading propose
